...
from global_utils.session_state_manager import (
    initialize_and_persist_filters,
)  # Used to manage filter state persistence.
...
taxonomy_file_default_path = (
    Path(__file__).parent / "databehandling/output/Andøya_fugl_taxonomy.csv"
)  # Defines the path to a local default CSV file.
...
innlastet_data = load_and_prepare_data(
    file_input_for_loader
)  # Loads and preprocesses data. Assumes file_input_for_loader is valid.
...
display_filter_widgets(
    innlastet_data
)  # Displays filter widgets. Assumes innlastet_data is a valid DataFrame for populating options.

# --- Apply Filters to Data ---
data_for_visning = apply_filters(
    innlastet_data
)  # Applies selected filters. Assumes filters are correctly applied to innlastet_data.
...
ikke_fremmede_arter_filtrert = (
    data_for_visning.copy()
)  # Default to all data as non-alien, will be overwritten if alien species are found.

# --- Build Alien Species Condition (Happy Path: assumes columns exist) ---
condition_alien_col_is_yes = (
    data_for_visning[alien_col].str.upper() == alien_identifier.upper()
)  # Condition for dedicated alien column. Assumes string comparison.
condition_category_is_alien = data_for_visning[category_col].isin(
    category_identifiers
)  # Condition for category column. Assumes .isin() works.
...
ikke_fremmede_arter_filtrert = data_for_visning[
    ~combined_alien_condition
]  # Applies inverse condition for non-alien species.
...
final_display_order = [
    col for col in preferred_display_order if col in existing_columns
]  # Filters preferred order by existing columns.
extra_columns = [
    col for col in existing_columns if col not in final_display_order
]  # Gets any remaining columns not in preferred list.
final_display_order.extend(extra_columns)  # Adds extra columns to the end.

hovedtabell_visning = hovedtabell_visning[
    final_display_order
]  # Reorders columns. Assumes all columns in final_display_order exist.
...
]  # Filters display order by existing alien columns.
fremmedart_tabell_visning = fremmedart_tabell_visning[
    existing_alien_columns
]  # Reorders columns. Assumes columns exist.
