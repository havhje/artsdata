...
from functions.birdnetlib_api import (
    run_birdnet_analysis, on_analyze_directory_complete
)

# Import functions from artskart_api
from functions.artskart_api import (
    fetch_artskart_taxon_info_by_name
)
...
output_csv_path = (
    script_dir / "data_output_lyd" / "interim" / "enriched_detections.csv"
)
...
def enrich_detections_with_taxonomy(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for index, row in df.iterrows():
        ...
        if taxon_info and isinstance(taxon_info, dict):
            ...
            if not species_nor_name and sci_name: # Added sci_name check for context
                logging.info(
                    f"No Norwegian species name found for '{sci_name}'. "
                    f"PopularNames from API: {popular_names_species}"
                )
    ...
    for fam_name in tqdm(unique_family_names, desc="Fetching Family Names", unit="fam"):
        if fam_name not in artskart_info_cache: # Check if already fetched (e.g. if a family name was also a species name)
        ...
        if taxon_info_fam:
            ...
            if not norwegian_name_for_family:
                logging.info(
                    f"No Norwegian name found for family '{fam_name}'. "
                    f"PopularNames from API: {popular_names_family}"
                )
        else:
    ...
    for ord_name in tqdm(unique_order_names, desc="Fetching Order Names", unit="ord"):
        ...
        if taxon_info_ord:
            ...
            if not norwegian_name_for_order:
                logging.info(
                    f"No Norwegian name found for order '{ord_name}'. "
                    f"PopularNames from API: {popular_names_order}"
                )
        else:
