...
def fetch_artskart_taxon_info_by_name(scientific_name_str: str) -> dict | None:
    ...
    try:
        ...
        if not isinstance(results, list):
            logging.warning(f"Unexpected API response type for '{scientific_name_str}'. Expected list, got {type(results)}.")
            return None

        if not results: # Empty list means no matches found
            logging.warning(f"No taxon found in Artskart for scientific name: '{scientific_name_str}'. API returned empty list.")
            return None
        ...
        for taxon_info in results:
            if isinstance(taxon_info, dict) and taxon_info.get("ValidScientificName") == scientific_name_str:
                logging.debug(f"Found exact match in Artskart for '{scientific_name_str}': ID {taxon_info.get('ValidScientificNameId')}")
            ...
            if isinstance(taxon_info, dict) and "ScientificNames" in taxon_info:
                for name_entry in taxon_info["ScientificNames"]:
                    if name_entry.get("ScientificName") == scientific_name_str and name_entry.get("Accepted") is True:
                        logging.debug(f"Found exact accepted match in Artskart (in ScientificNames list) for '{scientific_name_str}': ID {taxon_info.get('ValidScientificNameId')}")
                        return taxon_info
        ...
        logging.warning(f"No suitable match found for '{scientific_name_str}' even after checking first result heuristics.")
        return None
